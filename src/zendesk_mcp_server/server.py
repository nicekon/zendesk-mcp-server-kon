...
def build_tools() -> list[types.Tool]:
    return [
        ...
        types.Tool(name="zendesk_create_badge", description="Preview or create a Gather badge without an icon. Apply requires public-write gate and local approval.", inputSchema={"type": "object", "properties": {"badge_category_id": {"type": "string", "minLength": 1}, "name": {"type": "string", "minLength": 1}, "description": {"type": "string"}, "execution_mode": {"type": "string", "enum": ["preview", "apply"], "default": "preview"}, "approval_request_id": {"type": "string"}, "approval_token": {"type": "string"}}, "required": ["badge_category_id", "name", "description"]}),
        types.Tool(name="zendesk_update_badge", description="Preview or update a Gather badge without changing its icon. Apply requires public-write gate and local approval.", inputSchema={"type": "object", "properties": {"badge_id": {"type": "string", "minLength": 1}, "badge_category_id": {"type": "string", "minLength": 1}, "name": {"type": "string", "minLength": 1}, "description": {"type": "string"}, "execution_mode": {"type": "string", "enum": ["preview", "apply"], "default": "preview"}, "approval_request_id": {"type": "string"}, "approval_token": {"type": "string"}}, "required": ["badge_id"]}),
    ...
    async def handle_call_tool(
        ...
        if name == "zendesk_get_connection_status":
            ...
            if isinstance(tools, dict): result = tools
                ...
                values = arguments or {}; result = tools.create_badge(values.get("badge_category_id"), values.get("name"), values.get("description"), execution_mode=values.get("execution_mode", "preview"), approval_request_id=values.get("approval_request_id"), approval_token=values.get("approval_token"))
            elif name == "zendesk_update_badge":
                values = arguments or {}; badge = {field: values[field] for field in {"badge_category_id", "name", "description"} if field in values}; result = tools.update_badge(values.get("badge_id"), badge, execution_mode=values.get("execution_mode", "preview"), approval_request_id=values.get("approval_request_id"), approval_token=values.get("approval_token"))
