...
def build_tools() -> list[types.Tool]:
    tools = [
        ...
        types.Tool(name="zendesk_list_help_center_categories", description="List Help Center categories without making changes.", inputSchema={"type": "object", "properties": {"limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}, "cursor": {"type": "string", "minLength": 1}, "brand_id": {"type": "integer", "minimum": 1}, "locale": {"type": "string", "pattern": "^[a-z]{2,3}(-[a-z0-9]+)*$"}}}),
...
def create_server(environ: Mapping[str, str] | None = None) -> Server:
    ...
    async def handle_call_tool(
        ...
        if name == "zendesk_get_connection_status":
            ...
            elif name == "zendesk_list_help_center_categories": result = tools.list_categories(brand_id=(arguments or {}).get("brand_id"), locale=(arguments or {}).get("locale"), limit=(arguments or {}).get("limit", 100), cursor=(arguments or {}).get("cursor"))
            elif name == "zendesk_list_help_center_sections": result = tools.list_sections(brand_id=(arguments or {}).get("brand_id"), locale=(arguments or {}).get("locale"), limit=(arguments or {}).get("limit", 100), cursor=(arguments or {}).get("cursor"))
