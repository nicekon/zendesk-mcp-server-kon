...
def build_tools() -> list[types.Tool]:
    tools = [
        ...
        types.Tool(
            ...
            inputSchema={"type": "object", "properties": {"sort_by": {"type": "string", "enum": ["assignee", "assignee.name", "created_at", "group", "id", "requester", "requester.name", "status", "subject", "updated_at"], "description": "Selects offset pagination; resume using next_cursor with identical sorting. Maximum 10000 results."}, "sort_order": {"type": "string", "enum": ["asc", "desc"], "description": "Requires sort_by; defaults to asc."}, "limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}, "cursor": {"type": "string", "minLength": 1}, "sort": {"type": "string", "enum": ["id", "-id", "updated_at", "-updated_at", "status", "-status"]}}},
        ),
        types.Tool(
        ...
        types.Tool(
            ...
            inputSchema={"type": "object", "properties": {"source": {"type": "string", "enum": ["comments", "conversation_log", "auto"], "default": "comments", "description": "Auto detects the initial source from ticket messaging origin; resume using the returned source. Conversation log includes messaging and bot events. Returns data.events instead of data.comments; keep source unchanged when resuming."}, "ticket_id": {"type": "integer", "minimum": 1}, "limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}, "cursor": {"type": "string", "minLength": 1}}, "required": ["ticket_id"]},
        ),
        ...
        types.Tool(name="zendesk_search_users", description="Search Zendesk users, up to 1000 candidates per call and 10000 per query. Resume with the same query and next_cursor; offset pagination can shift when users change. Optional local filters may return fewer or no matches while has_more is true. Do not infer uniqueness before exhausting pages.", inputSchema={"type": "object", "properties": {"agents_only": {"type": "boolean", "default": False, "description": "Keep agents and admins only."}, "exact_name": {"type": "boolean", "default": False, "description": "Compare the returned full name to query, ignoring outer whitespace and case."}, "query": {"type": "string", "minLength": 1}, "limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}, "cursor": {"type": "string", "pattern": "^[0-9]{1,4}$"}}, "required": ["query"]}),
        ...
        types.Tool(name="zendesk_list_ticket_forms", description="List Zendesk ticket forms without making changes.", inputSchema={"type": "object", "properties": {"active": {"type": "boolean"}, "limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}, "cursor": {"type": "string", "minLength": 1}}}),
        ...
        types.Tool(name="zendesk_list_triggers", description="List Zendesk triggers with cursor pagination.", inputSchema={"type": "object", "properties": {"include_usage": {"type": "boolean", "default": False, "description": "Request usage for 1h, 24h, 7d and 30d. Requires a supported Zendesk plan; absent statistics are not zero."}, "active": {"type": "boolean"}, "category_id": {"type": "string", "minLength": 1}, "sort": {"type": "string", "enum": ["alphabetical", "created_at", "updated_at", "position"]}, "sort_order": {"type": "string", "enum": ["asc", "desc"]}, "limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}, "cursor": {"type": "string", "minLength": 1}}}),
        types.Tool(name="zendesk_list_help_center_categories", description="List Help Center categories without making changes.", inputSchema={"type": "object", "properties": {"sort_by": {"type": "string", "enum": ["position", "created_at", "updated_at"]}, "sort_order": {"type": "string", "enum": ["asc", "desc"]}, "limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}, "cursor": {"type": "string", "minLength": 1}, "brand_id": {"type": "integer", "minimum": 1}, "locale": {"type": "string", "pattern": "^[a-z]{2,3}(-[a-z0-9]+)*$"}}}),
        types.Tool(name="zendesk_list_help_center_sections", description="List Help Center sections without making changes.", inputSchema={"type": "object", "properties": {"category_id": {"oneOf": [{"type": "integer", "minimum": 1}, {"type": "string", "minLength": 1}]}, "sort_by": {"type": "string", "enum": ["position", "created_at", "updated_at"]}, "sort_order": {"type": "string", "enum": ["asc", "desc"]}, "limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}, "cursor": {"type": "string", "minLength": 1}, "brand_id": {"type": "integer", "minimum": 1}, "locale": {"type": "string", "pattern": "^[a-z]{2,3}(-[a-z0-9]+)*$"}}}),
        types.Tool(name="zendesk_search_help_center_articles", description="Search articles. Use locales with optional query and plural brand/category/section filters for Unified Search (50 per page); omit plural inputs for legacy offset search (1000-result cap). Never mix cursor types; resume with identical inputs.", inputSchema={"type": "object", "properties": {"limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}, "cursor": {"type": "string", "minLength": 1}, "query": {"type": "string", "minLength": 1}, "brand_id": {"type": "integer", "minimum": 1}, "locale": {"type": "string", "minLength": 2}, "locales": {"type": "array", "minItems": 1, "items": {"type": "string", "pattern": "^[a-z]{2,3}(-[a-z0-9]+)*$"}}, "brand_ids": {"type": "array", "minItems": 1, "items": {"type": "integer", "minimum": 1}}, "category_ids": {"type": "array", "minItems": 1, "items": {"oneOf": [{"type": "integer", "minimum": 1}, {"type": "string", "minLength": 1, "pattern": "^[^,]+$"}]}}, "section_ids": {"type": "array", "minItems": 1, "items": {"oneOf": [{"type": "integer", "minimum": 1}, {"type": "string", "minLength": 1, "pattern": "^[^,]+$"}]}}}, "oneOf": [{"required": ["locales"], "not": {"anyOf": [{"required": ["locale"]}, {"required": ["brand_id"]}]}}, {"required": ["query"], "not": {"anyOf": [{"required": ["locales"]}, {"required": ["brand_ids"]}, {"required": ["category_ids"]}, {"required": ["section_ids"]}]}, "properties": {"cursor": {"pattern": "^[0-9]{1,3}$"}}}]}),
        types.Tool(name="zendesk_export_help_center_articles", description="Export full articles to a managed JSON/CSV file. Omitted locale uses the selected brand default. Optional category_id or section_id scopes every page; section takes precedence. max_articles bounds the total.", inputSchema={"type": "object", "properties": {"locale": {"type": "string", "minLength": 2}, "category_id": {"oneOf": [{"type": "integer", "minimum": 1}, {"type": "string", "minLength": 1}]}, "section_id": {"oneOf": [{"type": "integer", "minimum": 1}, {"type": "string", "minLength": 1}]}, "brand_id": {"type": "integer", "minimum": 1}, "max_articles": {"type": "integer", "minimum": 1, "maximum": 100000}, "format": {"type": "string", "enum": ["json", "csv"], "default": "json"}}}),
        types.Tool(name="zendesk_get_help_center_article", description="Get an article, one active-locale translation, or all enabled-locale translations with locale=all. include_metadata adds related names; absent names remain null. Optional image embedding shares a 20 MB budget across translations.", inputSchema={"type": "object", "properties": {"article_id": {"oneOf": [{"type": "integer", "minimum": 1}, {"type": "string", "minLength": 1}]}, "brand_id": {"type": "integer", "minimum": 1}, "locale": {"type": "string", "minLength": 2}, "embed_images": {"type": "boolean", "default": False}, "include_metadata": {"type": "boolean", "default": False}}, "required": ["article_id"]}),
        ...
        types.Tool(name="zendesk_list_permission_groups", description="List Guide management permission groups using offset pagination. Resume cursors can shift when groups change.", inputSchema={"type": "object", "properties": {"limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}, "cursor": {"type": "string", "pattern": "^[0-9]{1,4}$"}}}),
        types.Tool(name="zendesk_list_user_segments", description="List Guide user segments without making changes.", inputSchema={"type": "object", "properties": {"built_in": {"type": "boolean"}, "applicable": {"type": "boolean", "default": False}, "limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}, "cursor": {"type": "string", "minLength": 1}}}),
        ...
        types.Tool(name="zendesk_list_community_votes", description="List Community votes for one post, post comment, or user (default 100, maximum 1000). Supply post_id and comment_id for a comment. User results include posts and post comments only, scanning at most 1000 raw votes per call. Resume next_cursor with the same scope.", inputSchema={"type": "object", "properties": {"post_id": {"type": "integer", "minimum": 1}, "comment_id": {"type": "integer", "minimum": 1}, "user_id": {"oneOf": [{"type": "integer", "minimum": 1}, {"type": "string", "enum": ["me"]}]}, "cursor": {"type": "string", "minLength": 1}, "limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}}, "oneOf": [{"required": ["post_id"], "not": {"required": ["user_id"]}}, {"required": ["user_id"], "not": {"anyOf": [{"required": ["post_id"]}, {"required": ["comment_id"]}]}}]}),
...
def _tool_annotations(name: str) -> types.ToolAnnotations:
    ...
    destructive = "_delete_" in name or name in {"zendesk_unassign_badge", "zendesk_remove_community_vote", "zendesk_replace_article_translation_body", "zendesk_update_ticket", "zendesk_set_ticket_status"}
...
def _tool_capability(name: str) -> str | None:
    ...
    if name in {"zendesk_list_permission_groups", "zendesk_list_user_segments"} or any(value in name for value in ("help_center", "article_translation", "guide_")): return "guide"
    if name in {"zendesk_search_users", "zendesk_list_groups", "zendesk_list_group_users", "zendesk_get_organization", "zendesk_list_brands", "zendesk_list_ticket_fields", "zendesk_list_ticket_forms", "zendesk_list_custom_statuses", "zendesk_list_views", "zendesk_get_view", "zendesk_list_view_tickets", "zendesk_list_macros", "zendesk_list_triggers", "zendesk_preview_macro", "zendesk_apply_macro"}: return "operations"
    return "support"
    ...
    async def handle_call_tool(
        ...
        if name == "zendesk_get_connection_status":
            ...
            if isinstance(tools, dict): result = tools
                ...
                values = arguments or {}; result = tools.list_votes(values.get("post_id"), comment_id=values.get("comment_id"), user_id=values.get("user_id"), cursor=values.get("cursor"), limit=values.get("limit", 100))
            ...
            elif name == "zendesk_assign_badge":
            ...
            elif name == "zendesk_unassign_badge":
            ...
            elif name == "zendesk_upload_community_image":
        ...
        elif name in {"zendesk_list_help_center_locales", "zendesk_list_help_center_categories", "zendesk_list_help_center_sections", "zendesk_search_help_center_articles", "zendesk_export_help_center_articles", "zendesk_get_help_center_article", "zendesk_get_satisfaction_ratings", "zendesk_list_csat", "zendesk_export_satisfaction_ratings", "zendesk_list_permission_groups", "zendesk_list_user_segments", "zendesk_create_help_center_article", "zendesk_upsert_article_translation", "zendesk_replace_article_translation_body", "zendesk_publish_help_center_article"}:
            tools = build_guide_tools(environment)
            if isinstance(tools, dict): result = tools
            elif name == "zendesk_list_help_center_locales": result = tools.list_locales(brand_id=(arguments or {}).get("brand_id"))
            elif name == "zendesk_list_help_center_categories": result = tools.list_categories(brand_id=(arguments or {}).get("brand_id"), locale=(arguments or {}).get("locale"), sort_by=(arguments or {}).get("sort_by"), sort_order=(arguments or {}).get("sort_order"), limit=(arguments or {}).get("limit", 100), cursor=(arguments or {}).get("cursor"))
            elif name == "zendesk_list_help_center_sections": result = tools.list_sections(brand_id=(arguments or {}).get("brand_id"), locale=(arguments or {}).get("locale"), sort_by=(arguments or {}).get("sort_by"), sort_order=(arguments or {}).get("sort_order"), category_id=(arguments or {}).get("category_id"), limit=(arguments or {}).get("limit", 100), cursor=(arguments or {}).get("cursor"))
            elif name == "zendesk_search_help_center_articles":
                values = arguments or {}; result = tools.search_articles(values.get("query"), brand_id=values.get("brand_id"), locale=values.get("locale"), locales=values.get("locales"), brand_ids=values.get("brand_ids"), category_ids=values.get("category_ids"), section_ids=values.get("section_ids"), limit=values.get("limit", 100), cursor=values.get("cursor"))
            elif name == "zendesk_export_help_center_articles":
                values = arguments or {}; result = tools.export_article_artifact(values.get("locale"), values.get("max_articles", 100000), brand_id=values.get("brand_id"), category_id=values.get("category_id"), section_id=values.get("section_id"), output_format=values.get("format", "json"))
            ...
            if isinstance(tools, dict):
                result = tools
            elif name == "zendesk_search_users": result = tools.search_users((arguments or {}).get("query"), (arguments or {}).get("limit", 100), cursor=(arguments or {}).get("cursor"), agents_only=(arguments or {}).get("agents_only", False), exact_name=(arguments or {}).get("exact_name", False))
            ...
            elif name == "zendesk_list_ticket_forms": result = tools.list_ticket_forms((arguments or {}).get("limit", 100), cursor=(arguments or {}).get("cursor"), active=(arguments or {}).get("active"))
            ...
            else: result = tools.list_triggers((arguments or {}).get("limit", 100), cursor=(arguments or {}).get("cursor"), active=(arguments or {}).get("active"), category_id=(arguments or {}).get("category_id"), sort=(arguments or {}).get("sort"), sort_order=(arguments or {}).get("sort_order"), include_usage=(arguments or {}).get("include_usage", False))
        elif name in {
            ...
            if isinstance(tools, dict):
                ...
                result = tools.list_tickets(limit, cursor=(arguments or {}).get("cursor"), sort=(arguments or {}).get("sort"), sort_by=(arguments or {}).get("sort_by"), sort_order=(arguments or {}).get("sort_order")) if isinstance(limit, int) and not isinstance(limit, bool) else failure(ErrorCode.VALIDATION_ERROR, "limit must be an integer")
