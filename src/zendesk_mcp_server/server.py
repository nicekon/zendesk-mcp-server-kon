...
def build_tools() -> list[types.Tool]:
    return [
        ...
        types.Tool(name="zendesk_list_community_comments", description="List comments for one Community post or user without making changes.", inputSchema={"type": "object", "properties": {"post_id": {"type": "integer", "minimum": 1}, "user_id": {"oneOf": [{"type": "integer", "minimum": 1}, {"type": "string", "enum": ["me"]}]}}}),
    ...
    async def handle_call_tool(
        ...
        if name == "zendesk_get_connection_status":
            ...
            if isinstance(tools, dict): result = tools
                ...
                values = arguments or {}; result = tools.list_posts(topic_id=values.get("topic_id"), user_id=values.get("user_id"), status=values.get("status"), sort_by=values.get("sort_by"))
                ...
                values = arguments or {}; result = tools.list_comments(values.get("post_id"), user_id=values.get("user_id"))
