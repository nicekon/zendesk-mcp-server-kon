...
def build_tools() -> list[types.Tool]:
    tools = [
        ...
        types.Tool(name="zendesk_list_triggers", description="List Zendesk triggers with cursor pagination.", inputSchema={"type": "object", "properties": {"active": {"type": "boolean"}, "category_id": {"type": "string", "minLength": 1}, "sort": {"type": "string", "enum": ["alphabetical", "created_at", "updated_at", "position"]}, "sort_order": {"type": "string", "enum": ["asc", "desc"]}, "limit": {"type": "integer", "minimum": 1, "maximum": 1000, "default": 100}, "cursor": {"type": "string", "minLength": 1}}}),
...
def create_server(environ: Mapping[str, str] | None = None) -> Server:
    ...
    async def handle_call_tool(
        ...
        if name == "zendesk_get_connection_status":
            ...
            else: result = tools.list_triggers((arguments or {}).get("limit", 100), cursor=(arguments or {}).get("cursor"), active=(arguments or {}).get("active"), category_id=(arguments or {}).get("category_id"), sort=(arguments or {}).get("sort"), sort_order=(arguments or {}).get("sort_order"))
        elif name in {
