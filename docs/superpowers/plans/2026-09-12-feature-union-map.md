...
2026-09-12, 최초 대조 구현 `8d37ce3ae8eb9c4d081d5b2fc765bc00c5ed2f9e` 기준.
이 표는 기능의 연결 관계이며 전체 동등성·실제 쓰기 검증 완료 선언이 아니다.
PRD 2.1·7·8과 승인된 안전성 변경이 기준이다. 도구 수 합산은 완료 기준이 아니다.

현재 읽는 방법 (`6f568cc` 후속 대조): 아래 G1–G8 누락표는 최초 발견 이력이며,
각 후속 구현 절에서 보완됐다. M10 offset 정렬도 마지막 절에서 보완됐다.
본문 중 “다음”, “아직 남음”은 해당 절 작성 당시의 상태다. 현재 구현을 중복 작업할
근거로 사용하지 않는다. 이 커밋에서 59개 대응 행의 모든 대상이 등록된 101개 도구에
존재하고 PRD 8장의 이름 집합과 정확히 일치함을 다시 확인했다. 이는 이름 연결의
증거이며 원본의 모든 동작·실계정 결과 동등성을 증명하지 않는다.
