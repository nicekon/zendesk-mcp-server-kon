...
`72d2b0c`의 PR #2 원격 CI `34689922391`에서 Python 3.10/3.11/3.12의
test→wheel→read-only gate→handshake와 macOS/Windows package smoke가 모두
성공했다. 아래 이전 PR revision 관련 문장은 당시 이력이다.

동일 코드로 지정 계정의 read-only 실조회도 수행했다. 인증 사용자 확인,
브랜드 목록(1개, 마지막 페이지), 해당 브랜드의 언어 설정, 배지 카테고리와
배지 목록의 첫 페이지(각 1개)가 성공했다. 고객 본문·이름·ID·token은 출력하지
않았으며 이미지 요청이나 운영 리소스 쓰기는 없었다. 이 계정에서 배지 조회가
가능함을 증명하지만 미구매 계정의 오류 유형이나 다른 역할의 권한은 증명하지
않는다. 실제 반환 브랜드가 하나이므로 다중 브랜드 실증으로 계산하지 않는다.
배지·카테고리는 각각 반환된 cursor로 두 번째 페이지까지 조회했고, 첫 페이지와
다른 ID의 항목 1개가 정상 반환됨을 확인했다. 전체 목록을 수집하거나 값을 출력하지
않았다. 이로써 두 리소스의 실계정 페이지 재개 근거를 추가했다.

### 현재 구현 감사: 375f023

2026-09-12에 PRD 14.2의 36개 조건, 기능 합집합의 후속 보완,
현재 코드와 테스트를 독립 읽기 전용 리뷰로 재대조했다. 추가로 확정된 필수
구현 누락은 없었다. 이는 아래 외부 수용 조건까지 통과했다는 선언이 아니다.

- OAuth 이전/롤백 도구가 추가됐으며, 원본 보존·최신 회전 토큰 유지·만료 백업
  처리·오류 비밀정보 제거를 구현했다. 공유 refresh lock의 링크 추적 취약점도
  실패 재현 후 수정하고 재검토했다. 35번의 근거는 이제 절차 문서만이 아니라
  `migration.py`와 `tests/test_migration.py`의 실제 파일 변환 회귀를 포함한다.
- 최신 구현의 전체 로컬 테스트: Python 3.10.17 **889 passed (9.33s)**,
  3.11.16 **889 passed (9.06s)**, 3.12.5 **889 passed (7.15s)**.
- 새 wheel의 격리된 site-packages import를 확인하고 repository pythonpath를
  제외한 설치 환경에서도 **889 passed (7.29s)**. 실제 Zendesk 요청은 없다.
- PR #2는 아직 `6ee8e98`이고 그 커밋의 Linux 3.10/3.11/3.12 및 macOS/Windows
  package CI는 성공했다. 이를 로컬 `375f023`의 원격 CI 성공으로 간주하지 않는다.

미해결 외부 수용 조건: 이미지 prepare_upload 403 및 실제 업로드 완료,
Badge 제품/역할별 판별 근거, 별도 sandbox 사람 승인 release check,
제품별·다중 브랜드·대량 데이터 실계정 검증과 실제 OAuth 이전/rollback 훈련.
테스트 계정 재요청이나 추가 운영 쓰기를 자동으로 수행하지 않는다.
이 조건을 우회하기 위해 일반 403을 제품 미지원으로 단정하거나 불필요한
기능·테스트를 계속 추가하지 않는다. 전체 PRD/출시 완료 판정은 보류한다.
