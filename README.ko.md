...
새 설치는 OAuth 로그인 후 티켓을 수정하고 Community 글·댓글을 작성할 수 있습니다. Community 작성은 항상 미리보기와 건별 로컬 승인이 필요하며, 삭제·대행 작성·외부 업로드는 기본으로 비활성 상태입니다.
...
기본 로그인은 검색에 필요한 읽기 권한과 기본 티켓·Community 기능에 필요한 `tickets:write`, `hc:write`를 요청합니다. 검색에 필요한 `read` 권한은 검색 외에도 사용자의 역할이 허용하는 데이터 조회에 적용됩니다.
...
| 수정이 차단됨 | `ZENDESK_WRITE_MODE=read_only`를 설정했는지 확인하세요. `ZENDESK_ENABLE_PUBLIC_WRITES=false`는 일반 티켓 수정은 허용하지만 Community 글·댓글 작성을 차단합니다. |
| `oauth_relogin_required` | 활성 기능에 지금 필요한 범위 없이 만든 OAuth 연결입니다. 같은 capability 설정으로 `zendesk login`을 다시 실행하세요. 토큰은 자동으로 변경하지 않습니다. |
